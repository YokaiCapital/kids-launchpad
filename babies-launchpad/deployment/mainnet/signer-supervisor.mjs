...
const env={...process.env,KIDS_SIGNER_KEY_FILE:keyFile,KIDS_SIGNER_PROGRAM_ID:process.env.KIDS_SIGNER_PROGRAM_ID||identities.program?.programId,KIDS_SIGNER_CAMPAIGNS:[process.env.KIDS_SIGNER_CAMPAIGNS,plan?.campaign].filter(Boolean).join(','),KIDS_SIGNER_HOST:process.env.KIDS_SIGNER_HOST||'::',KIDS_SIGNER_PORT:process.env.KIDS_SIGNER_PORT||process.env.PORT||'4176'};
