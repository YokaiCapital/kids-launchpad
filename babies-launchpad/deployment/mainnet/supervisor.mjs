...
import {mkdirSync,existsSync,readFileSync,writeFileSync} from 'node:fs';import {spawn} from 'node:child_process';import {createHash} from 'node:crypto';
...
// Operator key: from the KIDS_OPERATOR_KEY_JSON secret on first boot, then from the volume. The file path is what the code reads.
const keyFile=runtime+'/admin.json';
if(!existsSync(keyFile)){const raw=process.env.KIDS_OPERATOR_KEY_JSON;if(!raw)throw Error('KIDS_OPERATOR_KEY_JSON is required on first boot');const bytes=JSON.parse(raw);if(!Array.isArray(bytes)||bytes.length!==64)throw Error('Operator key must be a 64-byte array');writeFileSync(keyFile,JSON.stringify(bytes),{mode:0o600,flag:'wx'});}
process.env.KIDS_OPERATOR_KEY_FILE=keyFile;
const operator=Keypair.fromSecretKey(Uint8Array.from(JSON.parse(readFileSync(keyFile,'utf8'))));
const identities=JSON.parse(readFileSync(new URL('../MAINNET-IDENTITIES.json',import.meta.url),'utf8'));
if(profile.network==='mainnet'&&identities.operatorWallet?.address!==operator.publicKey.toBase58())throw Error('Operator key does not match the recorded operator wallet');
...
console.log(JSON.stringify({event:'program-verified',network:profile.network,programId,sha256,upgradeAuthority:authority,operator:operator.publicKey.toBase58(),operatorBalanceSol:(await connection.getBalance(operator.publicKey))/1e9}));
