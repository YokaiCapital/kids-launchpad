...
| Startup chain reconciliation before writes reopen | verified live | `startup-reconcile.mjs`, `runtime.mjs` | runtime test; hosted deployment 812ff1a5 restarted, writes reopened and passed the mixed check |
...
| Operator signing service | tested (desktop) | `operator-signer.mjs`, `signer-service.mjs` | 3 tests; lifecycle rehearsal with 22 remotely signed transactions; not deployed |
| Reproducible program build | verified (CI) | `.github/workflows/sbf-build.yml` | two runners, equal hash ef584ae7… |
| Alerts | implemented, delivery unconfirmed | `readiness.yml` | simulated failure run 35534207290; owner to confirm the notification |
...
| Mainnet program deployment and upgrade authority | open | | needs the authority decision and a mainnet deploy |
