...
const program=key('launch-escrow-program-key'),source=fileURLToPath(new URL('../programs/launch-escrow/Cargo.toml',import.meta.url)),out=directory+'/launch-escrow-build';mkdirSync(out,{recursive:true});
execFileSync(bin+'/cargo-build-sbf',['--manifest-path',source,'--sbf-out-dir',out],{stdio:'inherit'});
const binary=out+'/kids_launch_escrow.so',hash=createHash('sha256').update(readFileSync(binary)).digest('hex'),manifest=directory+'/launch-escrow-program.json';
...
}else execFileSync(bin+'/solana',['program','deploy',binary,'--program-id','launch-escrow-program-key.json','--keypair','admin.json','--url',config.rpcUrl,'--final'],{cwd:directory,stdio:'inherit'});
writeFileSync(manifest,JSON.stringify({programId:program.address,genesisHash:config.genesisHash,sha256:hash,source:'programs/launch-escrow/src/lib.rs',upgradeAuthority:null},null,2)+'\n',{mode:0o600});
