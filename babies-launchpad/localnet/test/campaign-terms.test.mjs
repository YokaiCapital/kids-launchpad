...
test('invalid terms refuse instead of falling back',()=>{
 for(const env of [{KIDS_ACTIVE_SOFT_CAP_SOL:'0'},{KIDS_ACTIVE_SOFT_CAP_SOL:'6',KIDS_ACTIVE_HARD_CAP_SOL:'5'},{KIDS_ACTIVE_HARD_CAP_SOL:'501'},{KIDS_ACTIVE_SOFT_CAP_SOL:'1e2'},{KIDS_ACTIVE_SOFT_CAP_SOL:'-1'},{KIDS_ACTIVE_DEADLINE_SECONDS:'59'},{KIDS_ACTIVE_DEADLINE_SECONDS:'604801'},{KIDS_ACTIVE_DEADLINE_SECONDS:'300.5'}])assert.throws(()=>activeCampaignTerms(env),Error,JSON.stringify(env));
});
test('manifest term bounds: positive soft, soft ≤ hard ≤ 500 SOL, fixed supply and 24 h launch window',()=>{
 ...
 for(const change of [{soft:'0'},{soft:'6000000000'},{hard:'500000000001'},{supply:'1'},{launchDeadline:86499},{soft:'x'}])assert.equal(validCampaignTerms({...base,...change}),false,JSON.stringify(change));
});
