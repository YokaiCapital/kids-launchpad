...
async function readPostlaunchState(scope='active'){
 ...
 return {configured:true,network:PROFILE.network,explorerUrl:PROFILE.explorerUrl,explorerCluster:PROFILE.explorerCluster,scope:selected.scope,genesisHash:ctx.manifest.genesisHash,programId:ctx.programId.toBase58(),campaign:selected.campaign.toBase58(),mint:campaign.mint.toBase58(),pool:campaign.pool.toBase58(),launchSignature:signature,feeEvents,receiptHistoryAvailable:selected.receiptHistoryAvailable,launchedAt:campaign.launchedAt,baseReserveRaw:(childFirst?reserve0:reserve1).toString(),quoteReserveLamports:(childFirst?reserve1:reserve0).toString(),supplyRaw:mint.supply.toString(),decimals:mint.decimals,tradeFeeBps:200,mintAuthorityRevoked:mint.mintAuthority===null,freezeAuthorityRevoked:mint.freezeAuthority===null,fees,liquidityLocked:null,liquidityLockQualified:true,claims:null,observedSlot:context.slot,observedAt:new Date().toISOString(),notice:selected.scope.startsWith('active-')?(PROFILE.network==='mainnet'?'Active Shartcoin campaign on Solana mainnet.':'Active Shartcoin campaign launched on isolated '+PROFILE.network+'. No mainnet funds.'):'Separate rehearsal coin. This is not the active Shartcoin campaign. Liquidity lock was verified at qualification; balances are read from the local chain.'};
}
