...
import {CPMM,LOCK,LOCK_AUTH,AMM_CONFIG,u64} from './atomic-launch.mjs';
...
export function convertFeesInstruction(ctx,campaign,keeper,mint,amount,minOutput,expiry){return swapFeesInstruction(ctx,campaign,keeper,mint,mint,NATIVE_MINT,amount,minOutput,expiry);}
...
function swapFeesInstruction(ctx,campaign,keeper,mint,inputMint,outputMint,amount,minOutput,expiry,parentIndex,outputProgram=TOKEN_PROGRAM_ID){
 const f=feeAddresses(ctx,campaign,mint),p=poolAddresses(CPMM,AMM_CONFIG,inputMint,outputMint),forward=p.mint0.equals(inputMint),parent=parentIndex!==undefined;
 ...
 const extra=[[ata(inputMint,f.authority),false,true],[ata(outputMint,f.authority,outputProgram),false,true],[p.pool,false,true],[AMM_CONFIG,false,false],[p.authority,false,false],[forward?p.vault0:p.vault1,false,true],[forward?p.vault1:p.vault0,false,true],[inputMint,false,false],[outputMint,false,parent],[p.observation,false,true],[CPMM,false,false],[TOKEN_PROGRAM_ID,false,false]];
