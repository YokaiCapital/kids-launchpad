...
const route=()=>({...(LOCAL_ADMIN?{admin:'Admin'}:{}),'shart-live':'PostLaunch','shart-preview':'PostLaunchPreview',shart:'Shart',launch:'Launch',vote:'Launch',submit:'Launch',archive:'Launch'}[location.hash.slice(1).split('/')[0]]||'Today');
...
export function App(){
 ...
 function go(next){const destination=['Vote','Submit','Archive'].includes(next)?'Launch':next;setView(destination);setModal(null);history.replaceState(null,'',`#${destination==='PostLaunch'?'shart-live':destination==='PostLaunchPreview'?'shart-preview':destination.toLowerCase()}`);window.scrollTo(0,0);}
 ...
 return <><header><button className="brand" aria-label="KIDS home" onClick={()=>go('Today')}><img className="brand-integrated" src="/assets/kids-logo-integrated-v1.png" alt="kids.fun"/></button><nav aria-label="Main navigation">{['Today','Shart','Launch'].map(v=><button key={v} className={`${(view===v||(['PostLaunch','PostLaunchPreview'].includes(view)&&v==='Shart'))?'active ':''}${v==='Launch'?'launch-nav':''}`} onClick={()=>go(v)}>{v==='Today'?'Explore':v==='Shart'?'Shartcoin':v}</button>)}<button onClick={()=>setModal('rules')}>Guide</button>{LOCAL_ADMIN&&<button onClick={()=>go('Admin')}>Admin</button>}</nav><div className="account"><button onClick={()=>setModal('allocations')}><Users size={22}/> My allocations</button><button className="outlined" onClick={()=>setModal('wallet')}><Wallet size={22}/><span>{identity?.owner?identity.owner.slice(0,4)+'…'+identity.owner.slice(-4):'Sign in'}</span></button></div></header>
 <main>{['PostLaunch','PostLaunchPreview'].includes(view)&&<PostLaunch key={view} preview={view==='PostLaunchPreview'} identity={identity} onSignIn={()=>setModal('wallet')} profile={coinProfile} posts={coinPosts} go={go}/>}{LOCAL_ADMIN&&view==='Admin'&&<Suspense fallback={null}><Admin/></Suspense>}{view==='Launch'&&<LaunchCountdown/>}{view==='Shart'&&<Prelaunch identity={identity} onSignIn={()=>setModal('wallet')} state={prelaunch} service={service} onRefresh={refreshRecords} profile={coinProfile} posts={coinPosts} onAction={persist} ready={serviceReady} go={go}/>}{view==='Today'&&<Home open={type=>type==='shart'?go('Shart'):setModal(type)} go={go}/>}</main>
 <footer><span>KIDS &nbsp; // &nbsp; SOLANA &nbsp; // &nbsp; TWO COMMUNITIES. ONE KID.</span><button className="text-button" onClick={()=>setModal('rules')}>Launch rules</button><span>{SITE_NETWORK==='mainnet'?'Solana mainnet · Live':SITE_NETWORK==='devnet'?'Devnet · Rehearsal':'Localnet · Not live'}</span></footer>
 ...
 {modal==='allocations'&&<><p>Hold at least 0.05% of either parent at its snapshot to qualify for that parent’s allocation.</p><p>No funded parent distribution is available yet.</p></>}
