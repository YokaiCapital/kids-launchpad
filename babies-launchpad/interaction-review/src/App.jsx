...
const route=()=>({...(LOCAL_ADMIN?{admin:'Admin'}:{}),'shart-live':'PostLaunch','shart-preview':'PostLaunchPreview',shart:'Shart',docs:'Docs',believers:'Believers',blocked:'Blocked',launch:'Launch',vote:'Launch',submit:'Launch',archive:'Launch'}[location.hash.slice(1).split('/')[0]]||'Today');
...
export function App(){
 ...
 return <><header><button className="brand" aria-label="KIDS home" onClick={()=>go('Today')}><img className="brand-integrated" src="/assets/kids-logo-integrated-v1.png" alt="kids.fun"/></button><nav aria-label="Main navigation">{['Today','Shart','Launch'].map(v=><button key={v} className={`${(view===v||(['PostLaunch','PostLaunchPreview'].includes(view)&&v==='Shart'))?'active ':''}${v==='Launch'?'launch-nav':''}`} onClick={()=>v==='Shart'?goCoin():go(v)}>{v==='Today'?'Explore':v==='Shart'?'Shartcoin':v}</button>)}<button onClick={()=>setModal('rules')}>Guide</button><button className={view==='Docs'?'active':''} aria-current={view==='Docs'?'page':undefined} onClick={()=>go('Docs','start')}>Docs</button><button className={['Believers','Blocked'].includes(view)?'active':''} aria-current={view==='Believers'?'page':undefined} onClick={()=>go('Believers')}>Believers</button>{LOCAL_ADMIN&&<button onClick={()=>go('Admin')}>Admin</button>}</nav><div className="account"><button onClick={()=>setModal('allocations')}><Users size={22}/> My allocations</button><button className="outlined" onClick={()=>setModal('wallet')}><Wallet size={22}/><span>{identity?.owner?identity.owner.slice(0,4)+'…'+identity.owner.slice(-4):'Sign in'}</span></button></div></header>
 ...
 <footer><span>KIDS &nbsp; // &nbsp; SOLANA &nbsp; // &nbsp; TWO COMMUNITIES. ONE KID.</span><button className="text-button" onClick={()=>setModal('rules')}>Launch rules</button><button className="text-button" onClick={()=>go('Docs','start')}>Docs</button><button className="text-button" onClick={()=>go('Blocked')}>Blocked wallets</button><span>{SITE_NETWORK==='mainnet'?'Solana mainnet · Live':SITE_NETWORK==='devnet'?'Devnet · Rehearsal':'Localnet · Not live'}</span></footer>
