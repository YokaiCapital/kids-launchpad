...
export function LocalTradePanel({owner,campaign,enabled,localExecution,mint,pool,programId,genesisHash,onSignIn,onTraded,wallet=null,network='localnet'}){
 ...
 <ArrowsDownUp size={18} className="post-swap-arrow"/><div className="post-receive"><span>You receive</span><strong>{display(quote?.outputRaw,side==='buy'?6:9)} <small>{side==='buy'?'$Shartcoin':'SOL'}</small></strong></div><div className="post-trade-fee"><span>Pool fee / Slippage</span><strong>2% / 1%</strong></div>
