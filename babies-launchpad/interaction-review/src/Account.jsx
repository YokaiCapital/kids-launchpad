...
export function Account({onChange}){
 ...
 {state?.owner?<><div className="kids-wallet-address">{state.owner}</div><button disabled={!!busy} onClick={()=>act('Signing out…',signOut)}>Disconnect & sign out</button></>:<div className="kids-wallet-list" aria-busy={!!busy}>{wallets.map(choice=><button key={choice.id} className={choice.paused?'kids-wallet-paused':''} disabled={!state||!!busy||!!choice.paused} aria-disabled={!!choice.paused} title={choice.paused||undefined} onClick={()=>act('Connecting '+choice.name+'…',()=>connect(choice))}>{choice.icon?.startsWith('data:image/')?<img src={choice.icon} alt=""/>:<span className="kids-wallet-monogram" aria-hidden="true">{choice.name[0]}</span>}<strong>{choice.name}</strong><span>{choice.paused||'Detected ↗'}</span></button>)}{!wallets.length&&<div className="kids-wallet-empty"><strong>No wallet detected yet</strong><p>Open this site in your wallet’s browser, or install a Solana wallet extension.</p><a href="https://solflare.com/download" target="_blank" rel="noreferrer">Get Solflare ↗</a><a href="https://backpack.app/download" target="_blank" rel="noreferrer">Get Backpack ↗</a><button onClick={()=>setWallets(discoverWallets())}>Check again</button></div>}</div>}
