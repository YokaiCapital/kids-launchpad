...
export function describeLaunch(data,nowUnix){const live=data?.network==='mainnet';
 ...
 if(data.phase==='open')return {phase:'open',tone:'live',pill:full?'Oversubscribed':'Open',headline:full?subscribed.toLocaleString('en-GB',{maximumFractionDigits:1})+'% subscribed. Still open.':reached?'Soft cap reached. Still open.':'Open for commitments',sub:full?'Allocation is proportional: each wallet\'s share of the coin follows its accepted SOL. Every new commitment dilutes everyone else\'s share; committing more keeps yours. Only '+progress.hard+' SOL goes into the pool; SOL above it is refunded pro rata after the close.':reached?'Up to the hard cap of '+progress.hard+' SOL, committed SOL joins the pool at launch. Anything above the hard cap is refunded pro rata after the close.':'Reach '+progress.soft+' SOL before the clock runs out and the coin launches by itself.',countdown:{label:'Closes in',seconds:closeIn,at:data.deadlineUnix},progress,addresses:null};
