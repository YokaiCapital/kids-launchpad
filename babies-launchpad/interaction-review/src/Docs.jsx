...
import {DOCS,docsFor,docHref,docText} from './docs-content';
...
export function Docs(){
 ...
 // The guide follows the live program: the API's programFeatures list picks the copy (build 6 buys and burns Shartcoin
 // and closes parent claims after 30 days). Until the read answers, today's copy is shown.
 const [features,setFeatures]=useState(null);
 useEffect(()=>{let active=true;accountApi('postlaunch').then(d=>{if(active)setFeatures(Array.isArray(d?.programFeatures)?d.programFeatures:[]);}).catch(()=>{});return()=>{active=false;};},[]);
 const docs=useMemo(()=>docsFor(features),[features]),searchText=useMemo(()=>docs.map(doc=>({doc,text:docText(doc).toLowerCase()})),[docs]);
