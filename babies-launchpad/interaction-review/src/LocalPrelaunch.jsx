...
export function LocalCommitPanel({chain,identity,onSignIn,endpoint='prelaunch',refundOnly=false,onOpenCoin=null}){
 ...
 {onOpenCoin&&state.phase==='launched'&&<div className="commit-confirmed launched-next" role="status"><strong>Shartcoin is live</strong><p>{committed>0n?'Your coins are ready to claim. Claiming and trading happen on the coin page.':'The pool is open. Trading happens on the coin page.'}</p><button className="text-button" onClick={onOpenCoin}>{committed>0n?'Claim and trade':'Go to trading'} →</button></div>}
