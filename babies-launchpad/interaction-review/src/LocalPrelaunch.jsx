...
export function LocalCommitPanel({chain,identity,onSignIn,endpoint='prelaunch',refundOnly=false,onOpenCoin=null}){
 ...
 <p className="small muted">{refundOnly?`Refunds follow the original close time: ${new Date(state.deadlineUnix*1000).toLocaleString()}`:open?`Closes ${new Date(state.deadlineUnix*1000).toLocaleString()}`:state.phase==='failed'?'Funding or launch requirements were not met. Full refunds are available.':state.phase==='awaiting-launch'?'Allocations are fixed. Pool launch is pending.':state.phase==='open'?'Commitments closed. Awaiting settlement.':'Pool launched.'}</p>
 ...
 {onOpenCoin&&state.phase==='launched'&&<div className="commit-confirmed launched-next" role="status"><strong>Shartcoin is live</strong><p>{committed>0n?'Your coins are ready to claim. Claiming and trading happen on the coin page.':'The pool is open. Trading happens on the coin page.'}</p><button className="primary" onClick={onOpenCoin}>{committed>0n?'Claim and trade Shartcoin':'Trade Shartcoin'}</button></div>}
 {!owner?<button className="primary" onClick={onSignIn}>Sign in to participate</button>:open?<button className="primary" disabled={busy||!!chain.error||!estimate} onClick={()=>transact('commit')}>{busy?phase:'Commit SOL'}</button>:refundable>0n?<button className="primary" disabled={busy||!!chain.error} onClick={()=>transact('refund')}>{busy?phase:'Claim refund'}</button>:<p className="small muted">{committed===0n?'No commitment from this wallet.':state.phase==='failed'?'Refund already processed.':'No refund available.'}</p>}
 ...
 {receipt?.owner===owner&&<div className="commit-confirmed" role="status" aria-live="polite"><strong>{receipt.action==='commit'?'Commitment confirmed':'Refund confirmed'}</strong>
  ...
  {receipt.action==='commit'&&<p>{open?`Funding closes ${new Date(state.deadlineUnix*1000).toLocaleTimeString()}. If the soft cap is met, the launch runs by itself and your allocation appears here.`:'Funding is closed. Settlement and launch run by themselves.'}</p>}
