...
export function LaunchStatus({data,go,onRefresh}){
 ...
 return <section className={'launch-status tone-'+d.tone} aria-live="polite">
  ...
  <p>{d.sub}</p>
  {d.progress&&<div className="launch-progress" role="img" aria-label={d.progress.raised+' SOL committed, soft cap '+d.progress.soft+' SOL, hard cap '+d.progress.hard+' SOL'}><div className="launch-bar"><div style={{width:d.progress.pct+'%'}}/><i style={{left:d.progress.softPct+'%'}}/></div><div className="launch-progress-labels"><span><strong>{d.progress.raised} SOL</strong> committed{d.progress.subscribed>0?' · '+d.progress.subscribed.toLocaleString('en-GB',{maximumFractionDigits:1})+'% of the hard cap':''}</span><span>Soft cap {d.progress.soft} SOL{d.progress.reached?' ✓':''}</span><span>Hard cap {d.progress.hard} SOL{d.progress.full?' ✓':''}</span></div></div>}
