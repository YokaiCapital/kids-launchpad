...
test('financial writes and readiness stay closed until the startup reconciliation gate opens; reads and other routes still work',async()=>{
 ...
 try{
  await app.check();
  let r=await fetch(url+'/_health/ready');assert.equal(r.status,503);assert.deepEqual((await r.json()).reconciliation,{complete:false,unresolvedSigned:2,at:'t0'});
