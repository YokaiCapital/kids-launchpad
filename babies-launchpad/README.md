...
| Admin settings, content, rounds and audit records | Separate operator authentication through the private gateway |
...
| External wallet claim signing | Implemented for registered launched localnet pools; rehearsal wallet-signed dev claim and replay verified |
| Multi-wallet connection | Wallet Standard discovery plus injected Phantom/Solflare/Backpack fallback; one selected wallet signs commits, refunds, claims and trades; bounded prompts and account-change guards; mock-wallet browser test passed, real extensions not yet qualified |
| Active post-launch selection | Implemented with phase-3/program/genesis/pool checks; long-lived campaign transition not yet exercised |
| Production authorization and automatic active fee operation | Pending |
...
Wallet signing keeps the browser-side validator unchanged: raw wallet-returned bytes are compared with the approved message, only bounded compute-budget additions are accepted, and there is no sign-and-send fallback. Wallet discovery through the Wallet Standard registry means a listed wallet is detected, not qualified.

## Hosted private test environment

See [private test status](deployment/PRIVATE-TEST-STATUS.md) for verified hosted localnet flows, capacity checks and remaining launch limitations.
