...
export function gatewayHeaders(method,path,role,secret,now=Date.now()){
 if(typeof secret!=='string'||secret.length<32||!['viewer','operator'].includes(role))throw Error('Gateway signing is not configured');
...
export function trustedGatewayContext(req,{port=4175,secret=process.env.KIDS_GATEWAY_INTERNAL_TOKEN,now=Date.now()}={}){
 ...
 if(typeof stamp!=='string'||!/^\d{13}$/.test(stamp)||Math.abs(now-Number(stamp))>15000||typeof nonce!=='string'||!/^[a-f0-9]{48}$/.test(nonce)||!['viewer','operator'].includes(role)||typeof signature!=='string'||!/^[a-f0-9]{64}$/.test(signature)||h.origin!==origin)return null;
 ...
 const context=Object.freeze({origin,operator:role==='operator'});cache.set(req,context);return context;
}
